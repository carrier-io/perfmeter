...
with open(script_path, "r", encoding="utf-8") as input_script, open(listener_path, "r") as listener_script:
    try:
        ...
        new_test = open("%s/%s_original.jmx" % (tests_path, test_name.split(".jmx")[0]), "w+", encoding="utf-8")
        new_test.write(test)
        new_test.close()
        test_to_run = open("%s/%s" % (tests_path, test_name), "w+", encoding="utf-8")
